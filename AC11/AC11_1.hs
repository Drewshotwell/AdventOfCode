...
addFlashes pastFlashedCoords eMap =
    let flashingCoords = (M.keys . M.filter (\v -> v == 0 || v > 9) $ eMap) \\ pastFlashedCoords
        resetEMap = foldl (\accM crd -> M.adjust (const 0) crd accM) eMap flashingCoords
        nextMap = 
            foldl (\accM crd ->
                foldl (\accMA crdA -> 
                    M.adjust (\v -> if v > 9 then 0 else v + 1) crdA accMA
                ) accM (adjacentCoords crd)
            ) resetEMap flashingCoords
        newFlashingCoords = pastFlashedCoords ++ (M.keys . M.filter (\v -> v == 0 || v > 9) $ nextMap)
    in  if 0 `elem` (M.elems nextMap) || any (>9) (M.elems nextMap) then
            addFlashes newFlashingCoords nextMap
        else
            nextMap
...
simulateOctopi :: M.Map (Int, Int) Int -> IO ()
simulateOctopi m = simulate window bgClr fps m modelToPicture stepFunction
   where 
      window = InWindow "Octopi or something" (screenDim, screenDim) (50, 50)
      bgClr = white
      fps = 1
      stepFunction _ _ = stepF
